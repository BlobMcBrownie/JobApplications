...
import cz.scilifapp.ui.devices.BLEActivity
...
class ConnectionManager (
    ...
    val devicesBonded = arrayListOf<BluetoothDevice?>(null, null, null, null, null, null)
    ...
    fun connect(device: BluetoothDevice) {
        if (device.isConnected()) {
            ...
            enqueueOperation(Connect(device, bleActivity.applicationContext))
        }
    }
    ...
    private fun doNextOperation() {
        ...
        when (operation) {
            is Disconnect -> with(operation) {
                ...
                resetDeviceCard(devicesBonded.indexOf(device))
                gatt.close()
                deviceGattMap.remove(device)
                devicesBonded[devicesBonded.indexOf(device)] = null
    ...
    private fun resetDeviceCard(index: Int) { // TODO ENDED HERE
        with (bleActivity) {
            setDeviceName("Device Name", index)
            setBatteryLevel(null, index)
            setBatteryCharging("0x00", index)
            setTemperature("??", index)
            setRGBValue(null, index)
            setLEDLightMode(0, index)
        }
    }
    ...
    private val callback = object : BluetoothGattCallback() {
        ...
        override fun onServicesDiscovered(gatt: BluetoothGatt, status: Int) {
            with(gatt) {
                if (status == BluetoothGatt.GATT_SUCCESS) {
                    val device = getKey(deviceGattMap, this)
                    Handler(Looper.getMainLooper()).post {
                        bleActivity.setDeviceName(device.name, devicesBonded.indexOf(device))
                    }
                    ...
                    saveCharacteristicsValues(device)
            ...
            with(characteristic) {
                when (status) {
                    ...
                    BluetoothGatt.GATT_INVALID_ATTRIBUTE_LENGTH -> {
                        Timber.e("Write exceeded connection ATT MTU! for $uuid")
                    }
                    else -> {
    ...
    private fun addDeviceToBonded(newDevice: BluetoothDevice?) {
        for ((index, device) in devicesBonded.withIndex()) {
            if (device == null) {
                devicesBonded[index] = newDevice
                return
            }
        }
    }

    private fun updateUI(uuid: UUID, value: ByteArray, gatt: BluetoothGatt) {
        when (uuid.toString()) {
            UUIDs().LCS_UUID_DIM_LED_CHAR -> { // external LED
                Handler(Looper.getMainLooper()).post {
                    bleActivity.setLEDLightMode(
                        value!!.toInt(),
                        devicesBonded.indexOf(getKey(deviceGattMap, gatt))
                    )
                }
            }
            UUIDs().LCS_UUID_DEBUG_LED_CHAR -> { // DEBUG RGB
                Handler(Looper.getMainLooper()).post {
                    bleActivity.setRGBValue(
                        value.toArrayOfIntStrings(),
                        devicesBonded.indexOf(getKey(deviceGattMap, gatt))
                    )
                }
            }
            UUIDs().MS_UUID_BATTERY_LEVEL_CHAR -> { // BATTERY LEVEL
                Handler(Looper.getMainLooper()).post {
                    bleActivity.setBatteryLevel(
                        value[0].toUByte().toString(),
                        devicesBonded.indexOf(getKey(deviceGattMap, gatt))
                    )
                }
            }
            UUIDs().MS_UUID_TEMP_CHAR -> { // TEMPERATURE
                Handler(Looper.getMainLooper()).post {
                    bleActivity.setTemperature(
                        value.toInt().toString(),
                        devicesBonded.indexOf(getKey(deviceGattMap, gatt))
                    )
                }
            }
            UUIDs().MS_UUID_BATTERY_CHARGE_CHAR -> {
                Handler(Looper.getMainLooper()).post {
                    bleActivity.setBatteryCharging(
                        value.toHexString(),
                        devicesBonded.indexOf(getKey(deviceGattMap, gatt))
                    )
                }
    ...
    fun saveCharacteristicsValues(device: BluetoothDevice) {
        val gatt = deviceGattMap[device]

        for (service in gatt!!.services) {
            for (characteristic in service.characteristics) {
                if (characteristic.uuid.toString() in UUIDs().CHARACTERISTICS) {
                    Timber.i("****** reading characteristic: ${characteristic.uuid}")
                    readCharacteristic(device, characteristic)
                    if (characteristic.uuid.toString() == UUIDs().LCS_UUID_DIM_LED_CHAR) {
    ...
    fun getBatteryLevel(device: BluetoothDevice) {
        getChar(device, UUIDs().MS_UUID_SERVICE, UUIDs().MS_UUID_BATTERY_LEVEL_CHAR)
    }

    fun getLEDValue(device: BluetoothDevice) {
        getChar(device, UUIDs().LCS_UUID_SERVICE, UUIDs().LCS_UUID_DIM_LED_CHAR)
    }

    fun getRGBValue(device: BluetoothDevice) {
        getChar(device, UUIDs().LCS_UUID_SERVICE, UUIDs().LCS_UUID_DEBUG_LED_CHAR)
    }

    fun getChar(device: BluetoothDevice, uuidService: String, uuidCharacteristic: String) {
        val gatt = deviceGattMap[device]
        val ServiceUUID = UUID.fromString(uuidService)
        val CharacteristicUUID = UUID.fromString(uuidCharacteristic)
        val Characteristic = gatt!!.getService(ServiceUUID)?.getCharacteristic(CharacteristicUUID)
        readCharacteristic(device, Characteristic!!)
    }

    fun writeLEDValue(device: BluetoothDevice, modeNum: Int) {
        var payload: ByteArray = modeNum.toByteArray()
        val gatt = deviceGattMap[device]
        val LEDServiceUuid = UUID.fromString(UUIDs().LCS_UUID_SERVICE)
        val ledCharUuid = UUID.fromString(UUIDs().LCS_UUID_DIM_LED_CHAR)
        val lightModeChar = gatt!!.getService(LEDServiceUuid)?.getCharacteristic(ledCharUuid)
        writeCharacteristic(device, lightModeChar!!, payload)
    }
